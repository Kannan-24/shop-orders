    ...
    <div class="bg-gray-100 min-h-screen py-0 *:lg:py-12">
        <div class="max-w-7xl mx-auto sm:px-0 lg:px-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <h1 class="text-3xl sm:text-4xl font-bold text-gray-800">Categories</h1>
            ...
            <form method="GET" action="{{ route('categories.index') }}" class="mb-6 flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3">
                ...
                @if(request('search'))
                    <a href="{{ route('categories.index') }}" class="text-blue-600 hover:underline w-full sm:w-auto text-center">Clear</a>
                @endif
            </form>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($categories as $category)
                            <tr class="hover:bg-gray-50 transition-all text-center">
                                <td class="px-2 sm:px-6 py-4 text-xs sm:text-base">{{ $loop->iteration + ($categories->currentPage() - 1) * $categories->perPage() }}</td>
                                <td class="px-2 sm:px-6 py-4 font-medium text-gray-900 text-xs sm:text-base">{{ $category->name }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-2 sm:px-6 py-4 text-center text-gray-500 text-xs sm:text-base">No categories found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
